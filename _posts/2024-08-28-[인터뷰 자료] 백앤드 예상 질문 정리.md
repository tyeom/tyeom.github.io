...
**10. RestFul 규약중 핵심**
> **Self-Descriptiveness**와 **HATEOAS** 원칙을 지키는 것이 중요하다.<br/><br/>
>
> **Self-descriptiveness(자기 기술성)** <br/>
> Self-descriptiveness는 Rest API 응답 결과 만으로도 그 의미를 해석할 수 있어야 함을 의미<br/><br/>
>
> **HATEOAS(Hypermedia As The Engine Of Application State)** <br/>
> HATEOAS는 Rest 아키텍처에서 중요한 특정으로 Hypermedia를 통해 어플리케이션 상태의 전이를 제어<br/>
> 즉 Rest API 응답 결과에 포함된 링크에 어플리케이션의 상태를 변경할 수 있어야 함.<br/><br/>
>
> **단순 JSON 결과는 위 두가지 요건을 충족시키지 못한다.** <br/>
> - Self-descriptiveness : JSON의 단순한 key-value 만으로 해당 key가 무엇을 의미하는지 알 수 없다.<br/>
> - HATEOAS : 상태로 전이할 링크 정보가 없다.<br/><br/><br/>
>
> 
> **Self-descriptiveness** 요건 만족을 위해 **Media type을 정의**하는데 JSON key-value를 정의하는 Media type을 정의한다.
...
{"id":1,"name":"잠실역"}
```
> **HATEOAS** 요건 만족을 위해 **link 헤더를 이용**해서 다음 상태를 표시할 수 있다.
...
> 위 결과는 지하철역 상세 정보 uri가 "/subways/detail/{id}" 에서 "/subways/{id}/detail"로 변경 되었을때 클라이언트 측에서는 일일히 기존 uri를 수정할 일 이 없다.
...
**20. Call by Sharing**
> javascript는 원시 값이 아닌 **참조 타입에 대해 함수 인자로 넘길때 참조값을 복사**해서 **복사본을 전달**한다.<br/>
> 때문에 다음과 같은 코드에서 유의해야 한다.
...
// 원시 타입으로 복사본을 만들어 전달함으로 변하지 않음
console.log(number);  // 1

// 참조 타입에 대해 복사본을 만들어
// 전달(객체의 같은 주소를 참조) 했고 해당 주소에 해당되는 객체의 속성을 변경하여 변경됨
console.log(object1);  // {name: 'changed'}

// 참조 타입에 대해 복사본을 만들어
// 전달(객체의 같은 주소를 참조) 했고 값 자체를 다른 객체로 대입했기에
// 참조하는 주소값이 변경되어 기존 객체의 속성값을 변하지 않음.
